...
const validateCreate = [
    ...
    check('subcategoria').not().isEmpty()
    .withMessage('La subcategoria es requerida'),

    check('clase').not().isEmpty()
    .withMessage('La clase es requerida'),

    check('unidad').not().isEmpty()
    .withMessage('La unidad es requerida')
    .custom((value) => {
        if (value.length > 50) {
            throw new Error('La unidad no puede tener mas de 50 caracteres');
        }
        return true;
    })
    .custom((value) => {
        if (value.length < 2) {
            throw new Error('La unidad debe tener al menos 2 caracteres');
        }
        return true;
    }),
